...
class WC_Straumur_Order_Handler {
    ...
    public function handle_cancellation(int $order_id) {
        ...
        if (!$order) {
            return new WP_Error('no_order', __('Invalid order.', 'straumur-payments'));
        }

        if ($order->get_payment_method() !== 'straumur') {
            return new WP_Error('not_straumur', __('Order not paid with Straumur.', 'straumur-payments'));
        }
        ...
        if (empty($payfac_reference) || empty($reference)) {
            return new WP_Error('no_reference', __('Cannot cancel: missing payment references.', 'straumur-payments'));
        }

        $api = new WC_Straumur_API();
        $this->logger->info("Attempting Straumur payment cancellation for order #{$order_id}", ['source' => 'straumur-payments']);
        ...
        if ($response) {
            ...
            $order->add_order_note(__('Straumur payment cancellation request sent', 'straumur-payments'));
            return true;
        }

        $this->logger->error("Cancellation request failed for order #{$order_id}", ['source' => 'straumur-payments']);
        return new WP_Error('cancellation_failed', __('Failed to send cancellation request to Straumur.', 'straumur-payments'));
    }
    ...
    public function handle_capture(int $order_id) {
        ...
        $reference        = $order->get_order_number();
        if (empty($payfac_reference) || empty($reference)) {
            return new WP_Error('no_reference', __('Cannot capture: missing payment references.', 'straumur-payments'));
        }
        ...
        if ($response) {
            ...
            $order->add_order_note(__('Straumur payment capture request sent', 'straumur-payments'));
            return true;
        }

        $this->logger->error("Capture request failed for order #{$order_id}", ['source' => 'straumur-payments']);
        return new WP_Error('capture_failed', __('Failed to send capture request to Straumur.', 'straumur-payments'));
    }
    ...
    public function handle_refund(int $order_id) {
        ...
        if (empty($payfac_reference) || empty($reference)) {
            return new WP_Error('no_reference', __('Cannot refund: missing payment references.', 'straumur-payments'));
        }
        ...
        if ($response) {
            ...
            $order->add_order_note(__('Straumur refund request has been sent to Straumur.', 'straumur-payments'));
        ...
        return new WP_Error('refund_failed', __('Failed to send refund request to Straumur.', 'straumur-payments'));
    }
}
